...
var musicList *container.Scroll		// 一定要传指针，否则组件刷新无效
var cs []fyne.CanvasObject
var Data AutoGenerated

var BasePath,_ = filepath.Abs(".")
var SavePath = binding.NewString()
var searchSubmit *widget.Button
var musicLength = 30

// music
var speedLabel *widget.Label
var speedSlider *widget.Slider
var musicStreamer *beep.Resampler		// 控制播放速度
var musicName = binding.NewString()		// 播放器标签
var musicCh = make(chan string, 1)		// 播放歌曲
var doneCh = make(chan bool, 1)			// 传递信息，歌曲播放完毕！
var ctrl *beep.Ctrl						// 控制播放/暂停
...
func randomPlay()  {
	...
	for {
		...
		case <-doneCh:
			...
			if len(Data) == 0 {
				dialog.ShowInformation("搜索失败!", "请更换关键词！", myW)
				break
			}
	...
	for {
		...
		case link = <-musicCh:
			...
			if err != nil || r.StatusCode != 200 {
				//log.Fatal(err)
				dialog.ShowInformation("链接失效!", "请重新搜索刷新数据！", myW)
				break
			}
			defer r.Body.Close()
...
func searchWidget(parent fyne.Window)fyne.CanvasObject  {
	...
	searchEntry.SetPlaceHolder("错位时空")
	...
	searchSubmit = widget.NewButtonWithIcon("搜索",theme.SearchIcon(), func() {
		...
		// 清空原有数据，重新渲染
		Data = musicAPI(API[searchEngine.Text], searchEntry.Text)
		cs = cs[0:0]
		for _,v := range Data {
			t := oneMusic(v.Name, v.Author, v.Src, parent)
			cs = append(cs, t)
