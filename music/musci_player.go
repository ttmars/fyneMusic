...
func MakeMusicPlayer() (c fyne.CanvasObject) {
	...
	mp.PlayerNext = widget.NewButtonWithIcon("", theme.MediaSkipNextIcon(), func() {
		if MyPlayer.PlayMode == 2 {
			rand.Seed(time.Now().Unix())
			randomNum := rand.Intn(len(MyPlayer.PlayList))
			if MyPlayer.CurrentSongIndex == randomNum {
				MyPlayer.MusicChan <- MyPlayer.PlayList[randomNum]
			}
			ml.Select(randomNum)
		} else {
			if MyPlayer.CurrentSongIndex == len(MyPlayer.PlayList)-1 {
				ml.Select(0)
			} else {
				ml.Select(MyPlayer.CurrentSongIndex + 1)
			}
		}
